...
function News() {
  ...
  console.log(key); // Should now log your key
  const newsFetch = async () => {
    try {
      const response = await fetch(
        `https://newsapi.org/v2/top-headlines?q=health&apiKey=${key}`
      );
 ...
 return (
   <div className="mt-10 container">
         <h2 className="text-xl text-blue-600 font-bold mb-2 text-center ">BETTER INFORMATION,BETTER HEALTH</h2>
         <h1 className="text-2xl font-bold mb-4 text-blue-950 text-center">NEWS</h1>
         {newsList.length > 0 ? (
             <div className="">
       <Swiper
         key={newsList.length}
         slidesPerView={2} // 2 columns
         grid={{ rows: 2, fill: "row" }} // 2 rows (fill horizontally first)
         spaceBetween={40}
         pagination={{ clickable: true }} //apply margin-top to pagination
         className="mySwiper"
         ...
         modules={[Grid, Pagination, Autoplay]}
            ...
            1024: {
                slidesPerView: 2, // 2 columns on large screens
                grid: { rows: 2, fill: "row" },
            },
            }}
       >
               ...
               <div className="flex flex-col justify-between lg:ml-4 items-start">
                 ...
                 <p >
                   {news.publishedAt?.slice(0, 10) || "No date"}
                             ...
                             <p className="italic">Author :{news.author || "Unknown author"}</p>
                         </div>
       ...
       <p>Loading news...</p> // <-- fallback while fetching
     )}
   </div>
 );
